...
export default function Hologram() {
  ...
  useEffect(() => {
    ...
    // Exact sizing logic from provided code
    const updateSize = () => {
      if (!containerRef.current) return { width: 550, height: 550 };
      return {
    ...
    // Lights
    const ambientLight = new THREE.AmbientLight(new THREE.Color(params.ambientLight), 0.5);
    scene.add(ambientLight);

    const dLight = new THREE.DirectionalLight(new THREE.Color(params.directionalLight), 1.0);
    ...
    // Clone materials for each index
    ...
    const animate = () => {
      ...
      icosahedron.rotation.y += delta * 0.3;
    ...
    // GUI - Hidden by default for clean look, but present as requested
    const gui = new GUI({ autoPlace: false });
    gui.domElement.style.position = "absolute";
    gui.domElement.style.top = "0px";
    gui.domElement.style.right = "0px";
    gui.domElement.style.zIndex = "100";
    gui.close(); // Keep it collapsed initially
    containerRef.current.appendChild(gui.domElement);

    gui.addColor(params, "color").name("Hologram Color").onChange((val) => {
      materials.forEach((m) => m.uniforms.uColor.value.set(val));
      pLight.color.set(val);
    });
    ...
    return () => {
      ...
      if (containerRef.current) {
        ...
        if (gui.domElement.parentNode === containerRef.current) {
          containerRef.current.removeChild(gui.domElement);
        }
      }
      gui.destroy();
      scene.traverse((node) => {
